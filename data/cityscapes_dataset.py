...
class CityscapesDataset(BaseDataset):
    ...
    A few source handpicked source images are used, and random images from the
    Cityscapes dataset as target images. To use, put --dataset_mode cityscapes
    ...
    def __init__(self, opt):
        ...
        if self.return_mask:
            ...
            self.mask_paths = glob(os.path.join(self.segment_dir,
                "*/*_color.png"))
            # check if the number of segmentation masks equals the nr of imgs
            assert len(self.mask_paths) == len(self.tgt_paths), f"Unequal nr \
            of images and segmentation masks ({len(self.tgt_paths)} & \
            {len(self.mask_paths)})"
    ...
    def __getitem__(self, index):
        ...
        C_transform = get_transform(self.opt, transform_params,
            grayscale=(self.output_nc == 1))
        ...
        out = {'src': A, 'tgt': B, 'irrel': C, 'src_paths': pathA,
                'tgt_paths': pathB, "irrel_paths": pathC}
        ...
        if self.return_mask:
            ...
            mask_transform = get_transform(self.opt, transform_params,
                grayscale=(self.output_nc == 1), method=Image.NEAREST)
            mask_visual = A_transform(mask)
            # no interpolation (use nn), to prevent new pixel values
